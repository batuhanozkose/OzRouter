...
// Check if settings has OzRouter config
const hasOzRouterConfig = (settings: any) => {
  ...
  return openai.some((p: any) => {
    if (p.name?.includes("OzRouter") || p.id === "ozrouter") return true;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      settings,
      hasOzRouter: hasOzRouterConfig(settings),
...
// POST - Write OzRouter config to settings.json + .env
export async function POST(request: Request) {
  ...
  try {
    ...
    const envLines = envContent.split("\n").filter((line) => {
      // Remove old OzRouter-related keys we're about to write
      return (
    ...
    // openai provider — primary, supports all models via OzRouter
    const openaiEntry = {
      id: resolvedModel,
      name: `${resolvedModel} (OzRouter)`,
    ...
    const openaiIdx = openaiProviders.findIndex(
      (p: any) => p && (p.baseUrl === normalizedBaseUrl || p.id === "ozrouter")
    );
    if (openaiIdx >= 0) {
    ...
    // anthropic provider — for Claude models via OzRouter
    ...
    // gemini provider — for Gemini models via OzRouter
    const geminiEntry = {
      id: "gemini-3-flash",
...
// DELETE - Remove OzRouter config from settings.json and .env
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove OzRouter entries from each provider type
    ...
    for (const type of providerTypes) {
      if (Array.isArray(existingConfig.modelProviders?.[type])) {
        existingConfig.modelProviders[type] = existingConfig.modelProviders[type].filter(
          (p: any) => !p.name?.includes("OzRouter") && p.id !== "ozrouter"
        );
    ...
    return NextResponse.json({
      success: true,
      message: "OzRouter settings removed from Qwen Code",
    });
  } catch (error) {
