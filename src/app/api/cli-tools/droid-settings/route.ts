...
// Check if settings has OzRouter customModels
const hasOzRouterConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some((m) => m.id === "custom:OzRouter-0");
};
...
// POST - Update OzRouter customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove existing OzRouter config if any
    settings.customModels = settings.customModels.filter((m) => m.id !== "custom:OzRouter-0");

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Add new OzRouter config
    const customModel = {
      model: model,
...
// DELETE - Remove OzRouter customModels only (keep other settings)
