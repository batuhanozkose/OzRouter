...
const hasOzRouterConfig = (settings: any) => {
  ...
  return !!settings.models.providers["ozrouter"];
};
...
// POST - Update OzRouter settings (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    settings.agents.defaults.model.primary = `ozrouter/${model}`;

    // Update models.providers.ozrouter
    settings.models.providers["ozrouter"] = {
...
// DELETE - Remove OzRouter settings only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove OzRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["ozrouter"];
    ...
    // Reset agents.defaults.model.primary if it uses ozrouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("ozrouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "OzRouter settings removed successfully",
    });
  } catch (error) {
