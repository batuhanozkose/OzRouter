...
// Check if OzRouter OpenAI-compatible provider is configured
const hasOzRouterConfig = (auth) => {
  if (!auth) return false;
  const routerEntry = auth["openai-compatible"] || auth["ozrouter"];
  if (!routerEntry) return false;
  const baseUrl = routerEntry.baseUrl || routerEntry.baseURL || "";
  return (
    baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("ozrouter")
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasOzRouter: hasOzRouterConfig(auth),
      authPath: AUTH_PATH,
    });
  } catch (error) {
...
// POST - Configure Kilo Code to use OzRouter as OpenAI-compatible provider
export async function POST(request) {
  ...
  try {
    ...
    // Add/update OzRouter as openai-compatible provider
...
// DELETE - Remove OzRouter config from Kilo
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove OzRouter provider
    delete auth["openai-compatible"];
    delete auth["ozrouter"];
    ...
    return NextResponse.json({
      success: true,
      message: "OzRouter settings removed from Kilo Code",
    });
  } catch (error) {
