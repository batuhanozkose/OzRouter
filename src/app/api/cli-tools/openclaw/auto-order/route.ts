...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [healthRes, combosRes] = await Promise.allSettled([
      fetch(`${OZROUTER_BASE_URL}/api/monitoring/health`, { signal: AbortSignal.timeout(5000) }),
      fetch(`${OZROUTER_BASE_URL}/api/combos`, { signal: AbortSignal.timeout(5000) }),
    ]);
