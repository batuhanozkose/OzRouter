...
// Required tables that must exist in a valid OzRouter database
...
export async function POST(request: Request) {
  ...
  try {
    ...
    tmpPath = path.join(os.tmpdir(), `ozrouter-import-${Date.now()}.sqlite`);
    ...
    try {
      ...
      if (missingTables.length > 0) {
        return NextResponse.json(
          {
            error: `Invalid OzRouter database. Missing tables: ${missingTables.join(", ")}`,
          },
          { status: 400 }
        );
      }
