...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tempDir = path.join(os.tmpdir(), `ozrouter-export-${timestamp}`);
    const zipPath = path.join(os.tmpdir(), `ozrouter-full-backup-${timestamp}.zip`);
    ...
    try {
      ...
      const metadata = {
        ...
        format: "ozrouter-full-backup-v1",
        contents: [
      ...
      return new NextResponse(archiveBuffer, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="ozrouter-full-backup-${timestamp}.tar.gz"`,
