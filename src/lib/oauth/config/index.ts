 ...
 * with the running OzRouter server when saving tokens.
 */
...
export function getServerCredentials(): ServerCredentials {
  return {
    server: process.env.OZROUTER_SERVER || process.env.SERVER_URL || getDefaultApiServer(),
    token: process.env.OZROUTER_TOKEN || process.env.CLI_TOKEN || "",
    userId: process.env.OZROUTER_USER_ID || process.env.CLI_USER_ID || "cli",
  };
}
