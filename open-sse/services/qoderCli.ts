...
export function getQoderCliWorkspace(): string {
  const explicit = String(
    process.env.QODER_CLI_WORKSPACE || process.env.OZROUTER_QODER_WORKSPACE || ""
  ).trim();
...
export function buildQoderPrompt(body: unknown): string {
  ...
  const lines = [
    "You are answering an OzRouter OpenAI-compatible request through the Qoder CLI transport.",
...
function buildCosyHeadersForValidation(bodyStr: string, token: string) {
  ...
  const uid = "ozrouter.user@qoder.sh";
  const userInfo = {
    uid: uid,
  ...
  try {
    ...
    return {
      ...
      error:
        ...
        "If behind a proxy, configure HTTPS_PROXY and verify the host has internet access.",
      unsupported: false,
    };
  }
