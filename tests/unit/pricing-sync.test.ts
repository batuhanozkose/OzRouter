...
import { transformToOzRouter } from "../../src/lib/pricingSync.ts";

// ─── transformToOzRouter ────────────────────────────────

describe("transformToOzRouter", () => {
  test("converts LiteLLM per-token pricing to OzRouter per-million format", () => {
    const raw = {
      "openai/gpt-4o": {
    ...
    const result = transformToOzRouter(raw);
...
describe("pricing merge precedence", () => {
  test("user overrides > synced > defaults conceptual order", () => {
    ...
    const synced = transformToOzRouter(raw);
