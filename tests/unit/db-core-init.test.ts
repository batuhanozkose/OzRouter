...
function cleanupGlobalDb() {
  try {
    if (globalThis.__ozrouterDb?.open) {
      globalThis.__ozrouterDb.close();
    }
  } catch {}
...
test("getDbInstance creates sqlite schema, metadata and applies migrations", serial, async () => {
  const dataDir = makeTempDir("ozrouter-db-core-");
  ...
  try {
    await withEnv({ DATA_DIR: dataDir, NEXT_PHASE: undefined }, async () => {
      ...
      const versions = db
        .prepare("SELECT version FROM _ozrouter_migrations ORDER BY version")
        .all()
        .map((row) => row.version);
...
test("module exports honor DATA_DIR from the environment", serial, async () => {
  const dataDir = makeTempDir("ozrouter-db-core-env-");
...
test(
  ...
  serial,
  async () => {
    const fakeHome = makeTempDir("ozrouter-home-");
        ...
        async () => {
          ...
          const expectedDir =
            process.platform === "win32"
              ? path.join(fakeHome, "AppData", "Roaming", "ozrouter")
              : path.join(fakeHome, ".ozrouter");
...
test("build phase uses an in-memory database without creating sqlite files", serial, async () => {
  const dataDir = makeTempDir("ozrouter-db-build-");
...
test("getDbInstance surfaces invalid DATA_DIR paths as sqlite open failures", serial, async () => {
  const sandboxDir = makeTempDir("ozrouter-db-bad-path-");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("ozrouter-db-legacy-empty-");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("ozrouter-db-legacy-data-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    createLegacySchemaDb(sqliteFile, { withData: true });
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("ozrouter-db-missing-max-concurrent-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    const seedDb = new Database(sqliteFile);
    const now = new Date().toISOString();
    ...
    seedDb.exec(`
      ...
      INSERT INTO _ozrouter_migrations (version, name) VALUES ('001', 'initial_schema');
      INSERT INTO _ozrouter_migrations (version, name) VALUES ('029', 'webhooks_templates');
    `);
    seedDb
      .prepare(
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("ozrouter-db-legacy-call-logs-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("ozrouter-db-calllogs-cache-source-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    const seedDb = new Database(sqliteFile);
    seedDb.exec(`
      CREATE TABLE _ozrouter_migrations (
      ...
      INSERT INTO _ozrouter_migrations (version, name) VALUES ('001', 'initial_schema');
      INSERT INTO _ozrouter_migrations (version, name) VALUES ('022', 'call_logs_cache_source');
    ...
    try {
      await withEnv({ DATA_DIR: dataDir, DISABLE_SQLITE_AUTO_BACKUP: "true" }, async () => {
        ...
        assert.ok(
          db.prepare("SELECT version FROM _ozrouter_migrations WHERE version = ?").get("022")
        );
        assert.ok(
          db.prepare("SELECT version FROM _ozrouter_migrations WHERE version = ?").get("023")
        );
        assert.ok(
          db.prepare("SELECT version FROM _ozrouter_migrations WHERE version = ?").get("026")
        );
        assert.ok(
          db.prepare("SELECT version FROM _ozrouter_migrations WHERE version = ?").get("027")
        );
        assert.equal(
          db
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("ozrouter-db-probe-recover-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("ozrouter-db-probe-latest-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("ozrouter-db-probe-abort-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
