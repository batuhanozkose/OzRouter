...
function withTempEnv(fn) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ozrouter-bootstrap-test-"));
  ...
  try {
    fn({ tempRoot, tempCwd, tempHome, dataDir: path.join(tempHome, ".ozrouter") });
  } finally {
...
test("bootstrapEnv prefers ~/.ozrouter/.env over server.env", () => {
  withTempEnv(({ dataDir }) => {
