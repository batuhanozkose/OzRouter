...
test("buildAliasEnvVar sanitizes aliases for env variable export", () => {
  assert.equal(buildAliasEnvVar("review"), "OZROUTER_MODEL_REVIEW");
  assert.equal(buildAliasEnvVar("plan mode"), "OZROUTER_MODEL_PLAN_MODE");
...
test("custom CLI generators include default model and alias mappings", () => {
  ...
  assert.match(envScript, /export OPENAI_MODEL="ozrouter\/fast"/);
  assert.match(envScript, /export OZROUTER_MODEL_REVIEW="cc\/claude-sonnet-4-5-20250929"/);
