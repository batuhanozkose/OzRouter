...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "ozrouter-a2a-enabled-route-"));
...
test.after(() => {
  ...
  if (ORIGINAL_OZROUTER_API_KEY === undefined) {
    delete process.env.OZROUTER_API_KEY;
  } else {
    process.env.OZROUTER_API_KEY = ORIGINAL_OZROUTER_API_KEY;
  }
});
...
test("A2A JSON-RPC checks auth before returning disabled state", async () => {
  process.env.OZROUTER_API_KEY = "test-secret";
