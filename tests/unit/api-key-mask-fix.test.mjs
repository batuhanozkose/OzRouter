...
describe("resolveApiKey", () => {
  ...
  it("falls back to sk_ozrouter when both are null", async () => {
    mockKeyStore.clear();

    const result = await resolveApiKey(null, null);
    assert.equal(result, "sk_ozrouter");
  });

  it("falls back to sk_ozrouter when both are undefined", async () => {
    mockKeyStore.clear();
