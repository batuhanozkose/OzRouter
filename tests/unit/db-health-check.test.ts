...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "ozrouter-db-health-check-"));
...
test("getDbInstance can auto-repair persisted broken rows when startup repair is forced", async () => {
  ...
  const previousForce = process.env.OZROUTER_FORCE_DB_HEALTHCHECK;
  process.env.OZROUTER_FORCE_DB_HEALTHCHECK = "1";
  try {
    db = core.getDbInstance();
  } finally {
    if (previousForce === undefined) {
      delete process.env.OZROUTER_FORCE_DB_HEALTHCHECK;
    } else {
      process.env.OZROUTER_FORCE_DB_HEALTHCHECK = previousForce;
    }
  }
