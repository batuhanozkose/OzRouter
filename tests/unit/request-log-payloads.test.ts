...
test("builds compact OpenAI stream summary for detailed logs", () => {
  ...
  assert.equal(compact._ozrouter_stream.stage, "provider_response");
  assert.equal(compact._ozrouter_stream.eventCount, 3);
...
test("builds compact Claude stream summary for detailed logs", () => {
  ...
  assert.equal(compact._ozrouter_stream.eventCount, 4);
});
