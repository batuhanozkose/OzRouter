...
test("status discrepancy shows both OzRouter and provider statuses", () => {
  const html = renderToStaticMarkup(
    React.createElement(RequestLoggerDetail, {
      log: {
        status: 504,
  ...
  assert.notEqual(
    html.indexOf("OzRouter returned 504"),
    -1,
    "Should indicate OzRouter returned its own status"
  );
});
