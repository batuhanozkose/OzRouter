...
const DUMMY_HOME = path.join(os.tmpdir(), "ozrouter-qwen-test-" + Date.now());
...
test("guide-settings POST creates new qwen settings.json if it doesn't exist", async () => {
  ...
  assert.equal(content.security?.auth?.baseUrl, "http://my-ozrouter");
...
test("guide-settings POST writes OpenCode config with current schema and multi-model selection", async () => {
  ...
  assert.equal(content.provider.ozrouter.npm, "@ai-sdk/openai-compatible");
  assert.equal(content.provider.ozrouter.options.baseURL, "http://my-ozrouter/v1");
  assert.ok(content.provider.ozrouter.options.apiKey.startsWith("sk-"));
  assert.deepEqual(Object.keys(content.provider.ozrouter.models), [
...
test("guide-settings POST preserves existing OpenCode config fields while only updating provider.ozrouter", async () => {
  await fs.mkdir(path.dirname(OPENCODE_CONFIG_PATH), { recursive: true });
  await fs.writeFile(
    OPENCODE_CONFIG_PATH,
    `{
  ...
  assert.equal(content.provider.ozrouter.options.baseURL, "http://my-ozrouter/v1");
  assert.ok(content.provider.ozrouter.options.apiKey.startsWith("sk-"));
  assert.deepEqual(content.provider.ozrouter.models, {
