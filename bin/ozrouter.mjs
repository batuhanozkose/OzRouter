...
/**
 * OzRouter CLI — Smart AI Router with Auto Fallback
 *
 * Usage:
...
function loadEnvFile() {
  ...
  if (home) {
    if (platform() === "win32") {
      ...
      envPaths.push(join(appData, "ozrouter", ".env"));
    } else {
      envPaths.push(join(home, ".ozrouter", ".env"));
    }
  }
...
if (args.includes("--help") || args.includes("-h")) {
  console.log(`
  \x1b[1m\x1b[36m⚡ OzRouter\x1b[0m — Smart AI Router with Auto Fallback
  ...
  \x1b[1mConfig:\x1b[0m
    ...
    Memory limit: OZROUTER_MEMORY_MB (default: 512)
...
if (args.includes("reset-encrypted-columns")) {
  ...
  \x1b[1mTo confirm, run:\x1b[0m
    ozrouter reset-encrypted-columns --force
    `);
    process.exit(0);
  }
...
if (!existsSync(serverJs)) {
  ...
  if (isMise) {
    console.error(
      "  \x1b[33m⚠ mise detected:\x1b[0m If you installed via `npm install -g ozrouter`,"
    );
    console.error("    try: \x1b[36mnpx ozrouter@latest\x1b[0m  (downloads a fresh copy)");
    console.error("    or:  \x1b[36mmise exec -- npx ozrouter\x1b[0m");
  } else if (isNvm) {
    console.error(
      "  \x1b[33m⚠ nvm detected:\x1b[0m Try reinstalling after loading the correct Node version:"
    );
    console.error("    \x1b[36mnvm use --lts && npm install -g ozrouter\x1b[0m");
  } else {
    console.error("  Try: \x1b[36mnpm install -g ozrouter\x1b[0m  (reinstall)");
    console.error("  Or:  \x1b[36mnpx ozrouter@latest\x1b[0m");
  }
  process.exit(1);
}
...
const rawMemory = parseInt(process.env.OZROUTER_MEMORY_MB || "512", 10);
const memoryLimit =
  Number.isFinite(rawMemory) && rawMemory >= 64 && rawMemory <= 16384 ? rawMemory : 512;

const env = {
  ...process.env,
  OZROUTER_PORT: String(port),
...
function shutdown() {
  console.log("\n\x1b[33m⏹ Shutting down OzRouter...\x1b[0m");
  server.kill("SIGTERM");
  setTimeout(() => {
    server.kill("SIGKILL");
    process.exit(0);
...
async function onReady() {
  ...
  console.log(`
  \x1b[32m✔ OzRouter is running!\x1b[0m
