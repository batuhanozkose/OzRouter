...
OzRouter is a local AI routing gateway and dashboard built on Next.js.
...
- MCP Server (29 tools) with 2 transports (SSE/Streamable HTTP)
...
flowchart LR
    ...
    subgraph Router[OzRouter Local Process]
...
- file: `${DATA_DIR}/storage.sqlite` (or `$XDG_CONFIG_HOME/ozrouter/storage.sqlite` when set, else `~/.ozrouter/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[OzRouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
- final client response returned by OzRouter; streamed responses are stored in the same compact summary form
...
1. `usageDb` and `localDb` share the same base directory policy (`DATA_DIR` -> `XDG_CONFIG_HOME/ozrouter` -> `~/.ozrouter`) with legacy file migration.
...
5. The `open-sse/` directory is published as the `@ozrouter/open-sse` **npm workspace package**. Source code imports it via `@ozrouter/open-sse/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
