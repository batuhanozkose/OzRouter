...
Complete reference for all OzRouter API endpoints.
...
| ----------------------- | --------- | ------------------------------------------------ |
| `X-OzRouter-No-Cache`   | Request   | Set to `true` to bypass cache                    |
| `X-OzRouter-Progress`   | Request   | Set to `true` for progress events                |
| `X-Session-Id`          | Request   | Sticky session key for external session affinity |
| `x_session_id`          | Request   | Underscore variant also accepted (direct HTTP)   |
| `Idempotency-Key`       | Request   | Dedup key (5s window)                            |
| `X-Request-Id`          | Request   | Alternative dedup key                            |
| `X-OzRouter-Cache`      | Response  | `HIT` or `MISS` (non-streaming)                  |
| `X-OzRouter-Idempotent` | Response  | `true` if deduplicated                           |
| `X-OzRouter-Progress`   | Response  | `enabled` if progress tracking on                |
| `X-OzRouter-Session-Id` | Response  | Effective session ID used by OzRouter            |
...
```json
{
  ...
  "backupPath": "/home/user/.ozrouter/backups/env-repair-2026-04-11.bak"
}
```
