/**
 * OzRouter Electron Desktop App - Main Process
 *
 ...
 * #14 Removed dead ozrouter:// protocol (no handler existed)
...
function resolveDataDir(overridePath, env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return path.join(appData, "ozrouter");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return path.join(path.resolve(xdg), "ozrouter");

  return path.join(require("os").homedir(), ".ozrouter");
}
...
function setupContentSecurityPolicy() {
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    ...
    const csp = [
      "default-src 'self'",
      `connect-src 'self' http://localhost:* ws://localhost:* https://*.github.com/batuhanozkose/OzRouter https://*.ozrouter.dev`,
      scriptSrc,
...
function startNextServer() {
  ...
  if (changed) {
    serverEnv.OZROUTER_BOOTSTRAPPED = "true";
    try {
      fs.mkdirSync(dataDir, { recursive: true });
      const lines = [
        "# Auto-generated by OzRouter bootstrap",
        "",
