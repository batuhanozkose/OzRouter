...
function normalizeEndpoint(rawEndpoint) {
  ...
  let parsed;
  try {
    parsed = new URL(endpoint, "http://ozrouter.local");
  } catch {
    return null;
  }

  if (parsed.origin !== "http://ozrouter.local") {
    return null;
  }
...
class WebSocketSession {
  ...
  async executeRequest(requestId, endpoint, payload, abortController) {
    const headers = {
      ...
      "content-type": "application/json",
      "x-ozrouter-ws-session-id": this.sessionId,
      "x-ozrouter-ws-request-id": requestId,
    };
...
export function createOzRouterWsBridge({
  baseUrl,
  fetchImpl = fetch,
  ...
  if (!isText(baseUrl)) {
    throw new Error("createOzRouterWsBridge requires a baseUrl");
  }
